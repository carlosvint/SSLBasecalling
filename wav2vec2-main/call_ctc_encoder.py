...
class Model(nn.Module):
    def __init__(self, label_vocab_size):
    ...
    def forward(self, signal, signal_lengths):
        ...
        out = self.final_proj(enc_output)
...
class Call(nn.Module):
    def __init__(self, opt):
        ...
        self.model = Model(label_vocab_size=model_opt.label_vocab_size)
...
def main():
    ...
    device = torch.device('cuda' if argv.device else 'cpu')
    argv.device = device
