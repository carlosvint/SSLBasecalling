...
def main_worker(gpu, ngpus_per_node):
    ...
    class Model(nn.Module):
        def __init__(self, checkpoint, label_vocab_size, quantizer=False):
            super(Model, self).__init__()
            self.quantizer = quantizer
            self.checkpoint = checkpoint
            self.feature_extractor = BasecallWav2Vec2(quantizer=False)
            self.feature_extractor.load_state_dict(checkpoint['model'])
            
            for p in self.feature_extractor.feature_extractor.parameters():
                p.requires_grad = True

            for p in self.feature_extractor.encoder.parameters():
                p.requires_grad = True

            for p in self.feature_extractor.project_q.parameters():
                p.requires_grad = False

            for p in self.feature_extractor.final_proj.parameters():
                p.requires_grad = False

            if self.quantizer == True:     
                for p in self.feature_extractor.quantizer.parameters():
                    p.requires_grad = False
        ...
        def forward(self, signal, signal_lengths):
            ...
            x = self.feature_extractor.feature_extractor(signal)
            enc_output, enc_output_lengths = self.feature_extractor.encoder(signal, signal_lengths, x)
