...
def main_worker(gpu, ngpus_per_node):
    ...
    class Model(nn.Module):
        def __init__(self, checkpoint, label_vocab_size, quantizer=False):
            ...
            self.feature_extractor = MAETConv()
            self.feature_extractor.load_state_dict(checkpoint['model_state_dict'])
            
            for p in self.feature_extractor.encoder.parameters():
                p.requires_grad = True

            #for p in self.feature_extractor.extrapolator.parameters():
            #    p.requires_grad = False
                
            #for p in self.feature_extractor.extrapolator_features.parameters():
            #    p.requires_grad = False    

            #for p in self.feature_extractor.upsample.parameters():
            #    p.requires_grad = False
            
            #for p in self.feature_extractor.decoder.parameters():
            #    p.requires_grad = False
            
            #for p in self.feature_extractor.pooling.parameters():
            #    p.requires_grad = False
        ...
        def forward(self, signal, signal_lengths):
            ...
            :return:
            """
            enc_output, enc_output_lengths = self.feature_extractor.encoder(signal, signal_lengths)
