            ...
            if raw_len + (e - s) <= raw_win_size:
                ...
                #     continue
                    ...
                    if raw == [] or seq == []:
                        ...
                        cheat_list.append(np.pad(
                            cheat, (0, raw_win_size-len(cheat)), mode='constant', constant_values=constants.SIG_PAD))
                        base_list.append(np.pad(
                            seq, (0, seq_win_size-len(seq)), mode='constant', constant_values=constants.PAD))
                        raw, seq, raw_len, cheat = init()
    return signal_list, base_list, cheat_list
...
def extract_file(input_file, raw_len, seq_len, signal_output_dir, label_output_dir, cheat_output_dir, basecall_group, basecall_subgroup):
    try:
    ...
    signal_list, base_list, cheat_list = splitdata(
    ...
    if signal_list != [] and base_list != []:
        ...
        np.save(cheat_output_dir+'/'+read_id, np.array(cheat_list))
...
def generate_train_data(input_dir, raw_len, seq_len, signal_output_dir, label_output_dir, cheat_output_dir, basecall_group='RawGenomeCorrected_000', basecall_subgroup='BaseCalled_template', n_thread=20):
    ...
    if not os.path.exists(label_output_dir):
        ...
        os.mkdir(label_output_dir)
    if not os.path.exists(cheat_output_dir):
        os.mkdir(cheat_output_dir)
    else:
        shutil.rmtree(cheat_output_dir)
        os.mkdir(cheat_output_dir)
    rawfile_list = [input_dir+'/'+v for v in os.listdir(input_dir)]
    extract_args = zip(rawfile_list, repeat(raw_len), repeat(seq_len), repeat(
        signal_output_dir), repeat(label_output_dir), repeat(cheat_output_dir), repeat(basecall_group), repeat(basecall_subgroup))
    pool = Pool(n_thread)
...
def run(args):
    ...
    generate_train_data(input_dir=FLAGS.input,
                        ...
                        cheat_output_dir=FLAGS.cheat_output,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-co', '--cheat_output', required=True)
